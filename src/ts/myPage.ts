...
function sessionView(): void {
    ...
    const user = sessionStorage.getItem("currentUser") === null ? null : JSON.parse(sessionStorage.getItem("currentUser"));
    if (user !== null) {
        const _logout = document.createElement("span") as HTMLSpanElement;
        loginForm.innerHTML = `<a href="myPage.html">${user.nickname}</a>`;
        _logout.onclick = () => {
            sessionStorage.clear();
            location.reload();
        }
        _logout.innerHTML = "로그아웃";
        loginForm.append(_logout);
    }
}
